function cart(db, printProducts, initialContet) {
    //let cart = [] 
    let cart = initialContet
    // Elementos del DOM
    ...
    function printCart() {
        ...
        let htmlCart = ''
        //Guardado de contenido de carrito en localstorage
        const ls = window.localStorage
        ls.setItem('cart', JSON.stringify(cart))


        //Generación contenido carrito
        if (cart.length === 0) {
            htmlCart += `
    ...
    function checkstock(productId) {
        ...
        cart.forEach((registro => {
            if (registro.id == productId) {
                numeroEnCarrito = registro.qty
            }
        }))
        //console.log("Numero en carrito", numeroEnCarrito)

        db.forEach((registro => {
            if (registro.id == productId) {
                numeroDisponibles = registro.quantity
            }
        }))
